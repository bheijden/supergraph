...
def _is_node_structural_match(
    ...
    """
    return host.in_degree(host_node_id) >= motif.in_degree(motif_node_id) and host.out_degree(host_node_id) >= motif.out_degree(motif_node_id)
...
def get_next_backbone_candidates(
    ...
    if len(required_edges) == 1:
        ...
        for (source, _, target) in required_edges:
            ...
            if candidate_nodes_set is None:
...
def find_motifs_iter(
    ...
    queue_ = queue_ or utils.Deque(policy=utils.QueuePolicy.BREADTHFIRST)
